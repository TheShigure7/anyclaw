...
func RegisterUIHandler(mux *http.ServeMux, scheduler *Scheduler, prefix string) {
	...
	mux.HandleFunc(prefix+"validate", func(w http.ResponseWriter, r *http.Request) {
		...
		desc, err := ValidateCronExpression(expr)
	...
	mux.HandleFunc(prefix+"next", func(w http.ResponseWriter, r *http.Request) {
		...
		expr := r.URL.Query().Get("expr")
		count := 5
		times, err := NextRunTimes(expr, time.Now(), count)
		resp := map[string]any{}
		if err != nil {
			resp["error"] = err.Error()
		} else {
...
func serveCronUI(w http.ResponseWriter, s *Scheduler) {
	tasks := s.ListTasks()
	stats := s.Stats()
...
func serveCronJSON(w http.ResponseWriter, s *Scheduler) {
	w.Header().Set("Content-Type", "application/json")
...
func handleCreateTask(w http.ResponseWriter, r *http.Request, s *Scheduler) {
	var task Task
