...
func TestAgentRunStreamPersistsAssistantResponse(t *testing.T) {
	mem := memory.NewFileMemory(t.TempDir())
	if err := mem.Init(); err != nil {
		t.Fatalf("memory init: %v", err)
	}

	llmStub := &stubAgentLLM{responses: []*llm.Response{{Content: "streamed response"}}}
	ag := New(Config{
		Name:        "assistant",
		Description: "General helper",
		LLM:         llmStub,
	...
	if err := ag.RunStream(context.Background(), "stream this", func(chunk string) {
	...
	if streamed.String() != "streamed response" {
		t.Fatalf("expected streamed response, got %q", streamed.String())
	}

	history := ag.GetHistory()
	if len(history) != 2 {
		t.Fatalf("expected history to include user and assistant messages, got %#v", history)
	}
	if history[1].Role != "assistant" || history[1].Content != "streamed response" {
		t.Fatalf("expected assistant stream result in history, got %#v", history[1])
	}

	conversation, err := mem.GetConversationHistory(0)
	if err != nil {
		t.Fatalf("GetConversationHistory: %v", err)
	}
	if len(conversation) != 2 {
		t.Fatalf("expected streamed exchange to be persisted, got %#v", conversation)
	}
	if conversation[1].Role != "assistant" || conversation[1].Content != "streamed response" {
		t.Fatalf("expected assistant stream result in memory, got %#v", conversation[1])
	}
}
