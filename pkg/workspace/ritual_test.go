...
func TestAdvanceBootstrapRitualCompletesAndRemovesBootstrapFile(t *testing.T) {
	...
	if result.PendingUserPrompt != "please help me" {
		t.Fatalf("expected pending prompt to be preserved, got %q", result.PendingUserPrompt)
	}
	if strings.Contains(result.Response, "Executing your earlier request") {
		t.Fatalf("expected bootstrap completion to defer execution, got %q", result.Response)
	}
