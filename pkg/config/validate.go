...
func (c *Config) Validate() error {
	...
	if c.Delegation.MaxDepth < 0 {
		errs = append(errs, fmt.Sprintf("delegation.max_depth must be >= 0 (got %d)", c.Delegation.MaxDepth))
	}
	if c.Delegation.PersistentSubagentSessionTTLMinutes < 0 {
		errs = append(errs, fmt.Sprintf("delegation.persistent_subagent_session_ttl_minutes must be >= 0 (got %d)", c.Delegation.PersistentSubagentSessionTTLMinutes))
	}
	if c.Delegation.TemporarySubagentTTLMinutes < 0 {
		errs = append(errs, fmt.Sprintf("delegation.temporary_subagent_ttl_minutes must be >= 0 (got %d)", c.Delegation.TemporarySubagentTTLMinutes))
	}
	validPersistentSubagentModelModes := map[string]bool{"": true, "inherit_main": true}
	validPersistentSubagentVisibility := map[string]bool{"": true, "internal": true, "internal_visible": true}
	validMarketSourceTypes := map[string]bool{"": true, "cloud": true, "local": true}
	for i, profile := range c.PersistentSubagents.Profiles {
		if strings.TrimSpace(profile.ID) == "" {
			errs = append(errs, fmt.Sprintf("persistent_subagents.profiles[%d].id is required", i))
		}
		if profile.PermissionLevel != "" && !validPermLevels[profile.PermissionLevel] {
			errs = append(errs, fmt.Sprintf("persistent_subagents.profiles[%d].permission_level must be one of: full, limited, read-only (got %q)", i, profile.PermissionLevel))
		}
		if !validPersistentSubagentModelModes[strings.TrimSpace(profile.ModelMode)] {
			errs = append(errs, fmt.Sprintf("persistent_subagents.profiles[%d].model_mode must be one of: inherit_main (got %q)", i, profile.ModelMode))
		}
		if !validPersistentSubagentVisibility[strings.TrimSpace(profile.Visibility)] {
			errs = append(errs, fmt.Sprintf("persistent_subagents.profiles[%d].visibility must be one of: internal, internal_visible (got %q)", i, profile.Visibility))
		}
	}
	for i, source := range c.Market.Sources {
		if strings.TrimSpace(source.ID) == "" {
			errs = append(errs, fmt.Sprintf("market.sources[%d].id is required", i))
		}
		if !validMarketSourceTypes[strings.TrimSpace(source.Type)] {
			errs = append(errs, fmt.Sprintf("market.sources[%d].type must be one of: cloud, local (got %q)", i, source.Type))
		}
	}
