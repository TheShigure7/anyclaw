...
func (c *WSClient) AbortChat(ctx context.Context) error {
	return c.AbortSessionChat(ctx, "")
}

func (c *WSClient) AbortSessionChat(ctx context.Context, sessionID string) error {
	frame := openClawWSFrame{
		Type:   "req",
		ID:     uniqueID("req"),
		Method: "chat.abort",
	...
	if strings.TrimSpace(sessionID) != "" {
		frame.Params = map[string]any{"session_id": strings.TrimSpace(sessionID)}
	}
	...
	if data, ok := resp.Data.(map[string]any); ok {
		if aborted, ok := data["aborted"].(bool); ok && !aborted {
			if reason, _ := data["reason"].(string); reason != "" {
				return fmt.Errorf("%s", reason)
			}
			return fmt.Errorf("chat abort failed")
		}
	}
	return nil
}
